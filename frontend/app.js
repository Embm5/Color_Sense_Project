...
// Botón: aplicar sólo Tritanopía (usa los `data-tritan-color` cuando existan)
if (tritanOnlyBtn) {
  tritanOnlyBtn.addEventListener("click", () => {
    daltonSelect.value = "tritanopia";
    updateColorCards("tritanopia");
    // force the flag CSS class for exact appearance
    const flag = document.getElementById("flagSim");
    if (flag) flag.classList.add("flag--tritan");
    // also update server output to reflect selection
    output.textContent = '{ "sim":"tritanopia" }';
    // scroll to flag for convenience
    if (flag) flag.scrollIntoView({ behavior: "smooth" });
  });
}

// Restaurar visión normal
if (restoreBtn) {
  restoreBtn.addEventListener("click", () => {
    daltonSelect.value = "normal";
    updateColorCards("normal");
    const flag = document.getElementById("flagSim");
    if (flag) flag.classList.remove("flag--tritan");
    output.textContent = "{ }";
